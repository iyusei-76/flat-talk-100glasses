...
def handle_im_messages(body, say, logger):
    ...
    # --- A-2. 「自分で設定する」で1on1相手のメンション入力待ちの場合はそちらを優先処理 ---
    if one_on_one.try_handle_pending_manual_partner(user_id, text, say):
        return

    # --- B. コマンドのルーティング（振り分け） ---

    # 1. 固定返答コマンドの場合
    if text in messages.STATIC_COMMANDS:
        say(messages.STATIC_COMMANDS[text])
